...
def show():
    ...
    if old_file is not None and new_file is not None:
        ...
        with col1:
            # 默认选中一些列名
            default_old_columns = ["所属区域", "母线名称"]  # 这里替换为你希望默认选中的列名
            old_columns = st.multiselect("选择原始数据表中的列", old_df.columns, default=default_old_columns)
        with col2:
            # 默认选中一些列名
            default_new_columns = ["所属区域", "母线名称"]  # 这里替换为你希望默认选中的列名
        ...
        if old_columns and new_columns:
            if len(old_columns) == len(new_columns):
                if st.button("开始数据处理"):
                    ...
                    # 将指定列设置为两位小数
                    if column_to_format in new_df.columns:
                        new_df[column_to_format] = new_df[column_to_format].apply(lambda x: '{:.2f}'.format(x))

                    column_to_format = '现电压偏差'
                        # # 检查列是否存在
                    if column_to_format in new_df.columns:
                            # 将列格式化为百分比形式
                        new_df[column_to_format] = new_df[column_to_format].apply(lambda x: f'{x * 100:.2f}%')
                    ...
                    with pd.ExcelWriter(output_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
                        ...
                        startrow = writer.sheets['母线'].max_row if writer.sheets['母线'].max_row > 0 else 0
                        new_df.to_excel(writer, sheet_name='母线', index=False, startrow=startrow, header=False)
