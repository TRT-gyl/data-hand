...
def show():
    ...
    if old_file is not None and new_file is not None:
        ...
        with col1:
            # 默认选中一些列名
            default_old_columns = ["所属区域", "设备/线路名称", "电压等级"]  # 这里替换为你希望默认选中的列名
            old_columns = st.multiselect("选择原始数据表中的列", filtered_df.columns, default=default_old_columns)
        with col2:
            # 默认选中一些列名
            default_new_columns = ["所属区域", "变电站名称", "电压等级"]  # 这里替换为你希望默认选中的列名
        ...
        if old_columns and new_columns:
            if len(old_columns) == len(new_columns):
                ...
                if st.button("开始数据处理"):
                    ...
                    new_df[column_name] = new_df[column_name].str.replace('高压侧', '')
